...
struct StructuredBuffer {
    ...
    private:
        ...
        protected:
            Element(Variant&& value) : m_value(std::move(value)) {
                NOTF_ASSERT(!m_value.valueless_by_exception());
        ...
        public:
            ...
            Element(const Element& other) : m_value(other.m_value), m_schema(other.m_schema) {
                NOTF_ASSERT(!other.m_value.valueless_by_exception());
                NOTF_LOG_DEBUG("Element copied");
            }

            Element(Element&& other) : m_value(std::move(other.m_value)), m_schema(std::move(other.m_schema)) {
                NOTF_ASSERT(!m_value.valueless_by_exception());
            }
            ...
            /// The type contained within this Element.
            Type get_type() const {
                switch (m_value.index()) {
                case get_first_index<number_t, Variant>(): return Type::NUMBER;
                case get_first_index<string_t, Variant>(): return Type::STRING;
                case get_first_index<list_t, Variant>(): return Type::LIST;
                case get_first_index<map_t, Variant>(): return Type::MAP;
                }
                NOTF_UNREACHABLE;
            }
    ...
    public:
        ...
        public:
            ...
            List(std::initializer_list<ingest<Element>> entries) : Element(_produce_list(std::move(entries))) {}

        private:
            static Variant _produce_list(std::initializer_list<ingest<Element>> entries) {
                NOTF_ASSERT(entries.size() != 0);
                list_t result = ingest_initializer_list(std::move(entries));

                const DynamicSchema reference_schema = result[0].get_schema();
                for (std::size_t i = 1; i < result.size(); ++i) {
                    if (result[i].get_schema() != reference_schema) {
        ...
        private:
            /// Type of an entry in the constructor's initializer list.
            using entry_t = typename map_t::value_type;
        ...
        public:
            ...
            Map(std::initializer_list<entry_t> entries) : Element(_produce_map(std::move(entries))) {}

        private:
            static Variant _produce_map(std::initializer_list<entry_t>&& entries) {
                NOTF_ASSERT(entries.size() != 0);

                // forcefully move all entries of the initializer list
                // I expect this to be save, since the pair contains l-values by definition
                map_t children;
                children.reserve(entries.size());
                for (const auto& entry : entries) {
                    children.emplace_back(std::move(const_cast<entry_t&>(entry)));
                }

                { // make sure that names are unique
                    std::unordered_set<std::string> unique_names;
                    for (const entry_t& child : children) {
...
void dynamic_test() {
    ...
    // using Schema = StructuredBuffer::DynamicLayout::DynamicSchema;
    using Type = StructuredBuffer::Type;

    auto testlist = List{0, 1, 2, 3, 4};

    // auto dynamic_number = String("averylondstringindedmyfriend");
    // static const auto static_number = String("averylondstringindedmyfriend");
    // static auto static_pair = std::make_pair(std::string("Derbe in the house y'all!"), String("this is a string element"));
    // auto testmap = Map{{"static", std::move(static_number)}, {"flexy", std::move(dynamic_number)}, static_pair};

    // clang-format off
    auto test_layout = List{
        Map{
            {"coords", 
                List{
                    Map{
    ...
    auto test_schema = test_layout.get_schema();
    assert(test_schema.size() == 13);
    assert(test_schema[0] == to_number(Type::LIST));
    assert(test_schema[1] == to_number(Type::MAP));
    assert(test_schema[2] == 3); // MapSize
    assert(test_schema[3] == 6); // Pointer to 6
    assert(test_schema[4] == to_number(Type::STRING));
    assert(test_schema[5] == 11); // Pointer to 11
    assert(test_schema[6] == to_number(Type::LIST));
    assert(test_schema[7] == to_number(Type::MAP));
    assert(test_schema[8] == 2); // MapSize
    assert(test_schema[9] == to_number(Type::NUMBER));
    assert(test_schema[10] == to_number(Type::STRING));
    assert(test_schema[11] == to_number(Type::LIST));
    assert(test_schema[12] == to_number(Type::STRING));
}
