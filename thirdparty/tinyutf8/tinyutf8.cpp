...
void utf8_string::raw_replace(size_type actual_start_index, size_type replaced_bytes, const utf8_string& replacement)
{
    ...
    if (indices_len) {
        ...
        if (indices_difference != 0) {
            for (
                size_type i                          = start_of_multibytes_in_range; i + number_of_multibytes_in_range < this->indices_len; i++)
    ...
    if (new_buffer_len > 1) {
        ...
        // Copy rest
        std::memcpy(
            new_buffer + actual_start_index + replacement_bytes, this->buffer + actual_end_index, normalized_buffer_len - actual_end_index - 1 // ('-1' for skipping the trailling zero which is not present in case the buffer_len was 0)
            );
