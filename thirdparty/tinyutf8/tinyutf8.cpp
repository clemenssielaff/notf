...
utf8_string::utf8_string(const char* str, size_type len) : utf8_string()
{
            ...
            if (this->misformatted)
                while (num_bytes_to_skip-- > 0) {
                    if (index + 1 == num_bytes || (static_cast<unsigned char>(buffer[index + 1]) & 0xC0) != 0x80) {
                        multibyteIndex--; // Remove the byte position from the list, since its no ++valid++ multibyte
                                          // sequence
                        break;
                    }
                    index++;
...
utf8_string::utf8_string(const value_type* str) : utf8_string()
{
    if (str && str[0]) {
...
unsigned char utf8_string::get_num_bytes_of_utf8_char_before(const char* data, size_type current_byte_index) const
{
    ...
    if (this->misformatted) {
        ...
        if ((static_cast<unsigned char>(data[-1]) & 0xC0) != 0x80 || current_byte_index < 2)
            return 1;
        // 110XXXXX - two bytes?
        if ((static_cast<unsigned char>(data[-2]) & 0xE0) == 0xC0)
            return 2;

        // Check if byte is no data-blob
        if ((static_cast<unsigned char>(data[-2]) & 0xC0) != 0x80 || current_byte_index < 3)
            return 1;
        // 1110XXXX - three bytes?
        if ((static_cast<unsigned char>(data[-3]) & 0xF0) == 0xE0)
            return 3;

        // Check if byte is no data-blob
        if ((static_cast<unsigned char>(data[-3]) & 0xC0) != 0x80 || current_byte_index < 4)
            return 1;
        // 11110XXX - four bytes?
        if ((static_cast<unsigned char>(data[-4]) & 0xF8) == 0xF0)
            return 4;

        // Check if byte is no data-blob
        if ((static_cast<unsigned char>(data[-4]) & 0xC0) != 0x80 || current_byte_index < 5)
            return 1;
        // 111110XX - five bytes?
        if ((static_cast<unsigned char>(data[-5]) & 0xFC) == 0xF8)
            return 5;

        // Check if byte is no data-blob
        if ((static_cast<unsigned char>(data[-5]) & 0xC0) != 0x80 || current_byte_index < 6)
            return 1;
        // 1111110X - six bytes?
        if ((static_cast<unsigned char>(data[-6]) & 0xFE) == 0xFC)
            return 6;
    }
    else {
...
unsigned char utf8_string::get_num_bytes_of_utf8_char(const char* data, size_type first_byte_index) const
{
    ...
    if (this->misformatted) {
        ...
        if ((static_cast<unsigned char>(data[1]) & 0xC0) != 0x80)
            return 1;
        // 110XXXXX - two bytes?
        ...
        // Check if byte is data-blob
        if ((static_cast<unsigned char>(data[2]) & 0xC0) != 0x80)
            return 1;
        // 1110XXXX - three bytes?
        ...
        // Check if byte is data-blob
        if ((static_cast<unsigned char>(data[3]) & 0xC0) != 0x80)
            return 1;
        // 11110XXX - four bytes?
        ...
        // Check if byte is data-blob
        if ((static_cast<unsigned char>(data[4]) & 0xC0) != 0x80)
            return 1;
        // 111110XX - five bytes?
        ...
        // Check if byte is data-blob
        if ((static_cast<unsigned char>(data[5]) & 0xC0) != 0x80)
            return 1;
        // 1111110X - six bytes?
...
utf8_string::size_type utf8_string::get_actual_index(size_type requested_index) const
{
    ...
    while (index_multibyte_table < this->indices_len) {
        ...
        current_overhead += get_num_bytes_of_utf8_char(this->buffer, multibyte_pos) - 1; // Ad bytes of multibyte to
                                                                                         // overhead
    }
...
void utf8_string::raw_replace(size_type actual_start_index, size_type replaced_bytes, const utf8_string& replacement)
{
    ...
    if (indices_len) {
        ...
        if (indices_difference != 0) {
            for (size_type i = start_of_multibytes_in_range; i + number_of_multibytes_in_range < this->indices_len; i++)
                new_indices[i + replacement_indices]
                    = this->indices_of_multibyte[i + number_of_multibytes_in_range] - byte_difference;
        }
        else {
    ...
    if (new_buffer_len > 1) {
        ...
        // Copy rest
        std::memcpy(new_buffer + actual_start_index + replacement_bytes, this->buffer + actual_end_index,
                    normalized_buffer_len - actual_end_index - 1 // ('-1' for skipping the trailling zero which is not
                                                                 // present in case the buffer_len was 0)
        );
...
int operator-(const utf8_string::const_iterator& lhs, const utf8_string::const_iterator& rhs)
{
    ...
    utf8_string::size_type num_codepoints
        = lhs.get_instance()->get_num_resulting_codepoints(minIndex, max_index - minIndex);
