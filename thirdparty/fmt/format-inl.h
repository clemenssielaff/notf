...
FMT_FUNC size_t internal::count_code_points(u8string_view s)
{
    ...
    for (size_t i = 0, size = s.size(); i != size; ++i) {
        if ((data[i].value & 0xc0) != 0x80) ++num_code_points;
    }
...
FMT_FUNC void grisu2_gen_digits(const fp& scaled_value, const fp& scaled_upper, uint64_t delta, char* buffer,
                                size_t& size, int& dec_exp)
{
    ...
    while (exp > 0) {
        ...
        if (digit != 0 || size != 0) buffer[size++] = static_cast<char>('0' + digit);
        --exp;
...
FMT_FUNC void grisu2_format_positive(double value, char* buffer, size_t& size, int& dec_exp)
{
...
FMT_FUNC void round(char* buffer, size_t& size, int& exp, int digits_to_remove)
{
    size -= to_unsigned(digits_to_remove);
    exp += digits_to_remove;
    int digit = buffer[size] - '0';
    if (digit > 5 || (digit == 5 && (digits_to_remove > 1 || (buffer[size - 1] - '0') % 2) != 0)) {
        ++buffer[size - 1];
    }
}
...
FMT_FUNC void format_exp_notation(char* buffer, size_t& size, int exp, int precision, bool upper)
{
...
FMT_FUNC void grisu2_prettify(char* buffer, size_t& size, int exp, int precision, bool upper)
{
    ...
    if (int_size <= full_exp && full_exp <= exp_threshold) {
        ...
        std::memmove(buffer + full_exp + 1, buffer + full_exp, to_unsigned(fractional_size));
        buffer[full_exp] = '.';
...
FMT_FUNC void
grisu2_format(double value, char* buffer, size_t& size, char type, int precision, bool write_decimal_point)
{
    FMT_ASSERT(value >= 0, "value is negative");
    int dec_exp = 0; // K in Grisu.
    if (value > 0) { grisu2_format_positive(value, buffer, size, dec_exp); }
    else {
        *buffer = '0';
        size = 1;
    }
    ...
    case 'f': {
        ...
        if (digits_to_remove > 0) {
            if (digits_to_remove >= static_cast<int>(size)) digits_to_remove = static_cast<int>(size) - 1;
            round(buffer, size, dec_exp, digits_to_remove);
        }
        break;
    }
    ...
    if (write_decimal_point && precision < 1) precision = 1;
...
FMT_FUNC void internal::format_windows_error(internal::buffer& out, int error_code, string_view message) FMT_NOEXCEPT
{
    FMT_TRY
    {
        ...
        for (;;) {
            ...
            int result = FormatMessageW(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, FMT_NULL,
