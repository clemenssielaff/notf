...
public:
    ...
    typename std::enable_if<std::is_integral<U>::value>::type operator()(U value)
    {
        ...
        if (const_check(sizeof(TargetType) <= sizeof(int))) {
            ...
            if (is_signed) { arg_ = internal::make_arg<Context>(static_cast<int>(static_cast<TargetType>(value))); }
            else {
...
template<typename OutputIt, typename Char,
         typename ArgFormatter = printf_arg_formatter<back_insert_range<internal::basic_buffer<Char>>>>
...
public:
    ...
    printf_arg_formatter(internal::basic_buffer<char_type>& buffer, format_specs& spec, context_type& ctx)
        : base(back_insert_range<internal::basic_buffer<char_type>>(buffer), &spec), context_(ctx)
    {}
    ...
    typename std::enable_if<std::is_integral<T>::value, iterator>::type operator()(T value)
    {
        ...
        if (std::is_same<T, bool>::value) {
            ...
            format_specs& fmt_spec = *this->spec();
            if (fmt_spec.type_ && fmt_spec.type_ != 'c') return (*this)(static_cast<int>(value));
            fmt_spec.flags_ = 0;
...
class basic_printf_context :
    ...
    public internal::context_base<OutputIt, basic_printf_context<OutputIt, Char, ArgFormatter>, Char> {
public:
...
basic_printf_context<OutputIt, Char, AF>::get_arg(iterator it, unsigned arg_index)
{
    (void)it;
    if (arg_index == std::numeric_limits<unsigned>::max()) return this->do_get_arg(this->parse_context().next_arg_id());
...
inline int vfprintf(std::FILE* f, basic_string_view<Char> format,
                    basic_format_args<typename printf_context<internal::basic_buffer<Char>>::type> args)
{
...
inline int vprintf(string_view format, printf_args args) { return vfprintf(stdout, format, args); }

inline int vprintf(wstring_view format, wprintf_args args) { return vfprintf(stdout, format, args); }
