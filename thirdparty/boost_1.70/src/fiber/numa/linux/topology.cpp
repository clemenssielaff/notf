...
std::set< std::uint32_t > ids_from_line( std::string const& content) {
    ...
    std::vector<std::string> v1 = split(content, ',');
    for ( std::string entry : v1) {
        trim( entry);
        std::vector<std::string> v2 = split(entry, '-');
        BOOST_ASSERT( ! v2.empty() );
        if ( 1 == v2.size() ) {
            // only one ID
            ids.insert( static_cast<std::uint32_t>(std::stoul( v2[0])) );
        } else {
            // range of IDs
            std::uint32_t first = static_cast<std::uint32_t>( std::stoul( * v2.begin() ));
            std::uint32_t last = static_cast<std::uint32_t>( std::stoul( * v2.rbegin() ));
...
std::vector< std::uint32_t > distance_from_line( std::string const& content) {
    ...
    for ( std::string entry : v1) {
        ...
        distance.push_back( static_cast<std::uint32_t>(std::stoul( entry )));
    }
    return distance;
}
...
std::vector< node > topology() {
    ...
    for ( std::uint32_t cpu_id : cpus) {
        std::stringstream ss;
        ss << "/sys/devices/system/cpu/cpu" << cpu_id << '/';
        fs::path cpu_path{ss.str()};
    ...
    for ( auto entry : map) {
        // NUMA-node distance
        std::stringstream ss;
        ss << "/sys/devices/system/node/node" << entry.second.id << "/distance";
        fs::path distance_path{ss.str()};
