...
private:
    ...
    bool eval_(fs::directory_entry const& entry)
    {
        ...
        std::smatch what;
        if (!std::regex_search(filename, what, exp_)) { return false; }
...
public:
    ...
    directory_iterator(fs::path const& dir, std::string const& exp) : i_(dir), e_(), exp_(exp), idx_()
    {
...
std::vector<node> topology()
{
    ...
    for (std::uint32_t cpu_id : cpus) {
        fs::path cpu_path{fmt::format("/sys/devices/system/cpu/cpu{}/", cpu_id)};
    ...
    for (auto entry : map) {
        // NUMA-node distance
        fs::path distance_path{fmt::format("/sys/devices/system/node/node{}/distance", entry.second.id)};
