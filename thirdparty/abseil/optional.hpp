...
// Note by Clemens Sielaff, March 20th 2018
// This is a modified version of the files:
//  absl/types/bad_optional_access.h
//  absl/types/bad_optional_access.cc
//  absl/types/optional.h
//  absl/types/optional.cc
// from abseil repository at commit: 0ec11bad6fc18822e851b25344f027491cc85746
// that has been made standalone except for the NoTF meta include.
#pragma once
...
// throw delegator
[[noreturn]] inline void throw_bad_optional_access() { throw bad_optional_access(); }
...
protected:
    ...
    template<typename... Args>
    constexpr explicit optional_data_base(in_place_t t, Args&&... args) : base(t, std::forward<Args>(args)...)
    {}
#endif
...
template<typename T, bool = std::is_trivially_copy_constructible<T>::value&& std::is_trivially_copy_assignable<
...
protected:
    ...
    constexpr explicit optional_data(in_place_t t, Args&&... args)
        : optional_data_base<T>(t, std::forward<Args>(args)...)
    {}
#endif
};
...
{
    return std::is_copy_assignable<T>::value && std::is_copy_constructible<T>::value ?
               copy_traits::copyable :
               std::is_move_assignable<T>::value && std::is_move_constructible<T>::value ? copy_traits::movable :
...
struct optional_hash_base<T, decltype(std::hash<std::remove_const_t<T>>()(std::declval<std::remove_const_t<T>>()))> {
    ...
    {
        if (opt) {
            return std::hash<std::remove_const_t<T>>()(*opt);
        }
        else {
...
public:
    ...
    T&& value() &&
    { // NOLINT(build/c++11)
        return std::move(static_cast<bool>(*this) ? reference() :
    ...
    {
        static_assert(std::is_copy_constructible<value_type>::value, "optional<T>::value_or: T must by copy "
                                                                     "constructible");
    ...
    T value_or(U&& v) &&
    { // NOLINT(build/c++11)
        static_assert(std::is_move_constructible<value_type>::value, "optional<T>::value_or: T must by copy "
                                                                     "constructible");
        static_assert(std::is_convertible<U&&, value_type>::value, "optional<T>::value_or: U must be convertible to T");
...
private:
    ...
    // or a reference.
    static_assert(!std::is_same<nullopt_t, typename std::remove_cv<T>::type>::value, "optional<nullopt_t> is not "
                                                                                     "allowed.");
    static_assert(!std::is_same<in_place_t, typename std::remove_cv<T>::type>::value, "optional<in_place_t> is not "
                                                                                      "allowed.");
...
constexpr auto operator!=(const optional<T>& x, const optional<U>& y)
    -> decltype(optional_internal::convertible_to_bool(*x != *y))
{
    return static_cast<bool>(x) != static_cast<bool>(y) ? true : static_cast<bool>(x) == false ? false : *x != *y;
}
