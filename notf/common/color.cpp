...
/// See dlib_license.txt for the license.
...
NOTF_UNUSED Hsl rgb_to_hsl(const Color& input) {
    ...
    result.s = 0;
    if (result.l > 0 && result.l < 1) { result.s = delta / (result.l < 0.5f ? (2 * result.l) : (2 - 2 * result.l)); }
    result.h = 0;
    if (delta > 0) {
#ifdef __clang__
...
#endif
        if (themax == input.r && themax != input.g) { result.h += (input.g - input.b) / delta; }
        if (themax == input.g && themax != input.b) { result.h += (2 + (input.b - input.r) / delta); }
        if (themax == input.b && themax != input.r) { result.h += (4 + (input.r - input.g) / delta); }
#ifdef __clang__
...
Color::Color(const std::string& value) {
    if (!is_color(value)) { NOTF_THROW(ValueError, "\"{}\" is not a valid color value", value); }
...
std::string Color::to_string() const {
    ...
    buffer << "#" << std::hex << static_cast<int>(roundf(r * 255.f)) << static_cast<int>(roundf(g * 255.f))
           << static_cast<int>(roundf(b * 255.f)) << static_cast<int>(roundf(a * 255.f));
    return buffer.str();
}
