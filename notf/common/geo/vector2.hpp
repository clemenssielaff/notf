...
public:
    ...
    template<class... Args>
    constexpr Vector2(Args&&... args) noexcept : super_t(std::forward<Args>(args)...) {}

    /// Unit vector along the X-axis.
    static constexpr Vector2 x_axis() noexcept { return Vector2(1, 0); }
    ...
    Vector2 get_rotated(const element_t angle) const noexcept {
        const element_t sin = fast_sin(angle);
        const element_t cos = fast_cos(angle);
    ...
    Vector2 get_rotated(const element_t angle, const Vector2& pivot) const noexcept {
