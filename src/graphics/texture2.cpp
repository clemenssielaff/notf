...
Texture2::load_image(GraphicsContext& context, const std::string file_path, const Args& args) {
	...
	std::shared_ptr<Texture2> texture
	    = Texture2::_create(id, context, std::move(file_path), image_size.width, image_size.height, texture_format);
	context.m_textures.emplace_back(texture);
	return texture;
}

std::shared_ptr<Texture2>
Texture2::create_empty(GraphicsContext& context, const std::string name, const Size2i& size, const Args& args) {
	if (!size.is_valid()) {
	...
	// return the loaded texture on success
	std::shared_ptr<Texture2> texture
	    = Texture2::_create(id, context, std::move(name), size.width, size.height, args.format);
	context.m_textures.emplace_back(texture);
	return texture;
}
...
Texture2::Texture2(const GLuint id, GraphicsContext& context, const std::string name, const GLint width,
                   const GLint height, const Format format)
    : m_id(id)
