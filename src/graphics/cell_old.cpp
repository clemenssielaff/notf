...
void Cell_Old::reset(const RenderContext_Old& context)
{
    m_states.clear();
...
void Cell_Old::fill(RenderContext_Old& context)
{
...
void Cell_Old::stroke(RenderContext_Old& context)
{
    const RenderState& state = _get_current_state();
...
void Cell_Old::_expand_stroke(const float stroke_width)
{
    ...
    for (Path& path : m_paths) {
        ...
        if (!path.is_closed) {
            // add cap
            const Vector2f direction = m_points[previous_offset].forward;
            switch (line_cap) {
            case LineCap::BUTT:
        ...
        if (path.is_closed) {
            ...
            // add cap
            const Vector2f delta = m_points[previous_offset].forward;
            switch (line_cap) {
            case LineCap::BUTT:
