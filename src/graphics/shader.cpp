...
public: // methods
	/** Default constructor. */
	ShaderRAII() : m_shader(0), m_program(0) {}

	/** Value constructor. */
	ShaderRAII(GLuint shader) : m_shader(shader), m_program(0) {}
...
Shader::load(GraphicsContext& context, const std::string& name, const std::string& vertex_shader_file,
             const std::string& fragment_shader_file, const std::string& geometry_shader_file) {
...
std::shared_ptr<Shader>
Shader::build(GraphicsContext& context, const std::string& name, const std::string& vertex_shader_source,
              const std::string& fragment_shader_source, const std::string& geometry_shader_source) {
	// compile the shaders
...
Shader::Shader(const GLuint id, GraphicsContext& context, const std::string name)
    : m_id(id), m_graphics_context(context), m_name(std::move(name)), m_uniforms(), m_attributes() {
	...
	{ // discover uniforms
		...
		for (GLuint index = 0; index < static_cast<GLuint>(uniform_count); ++index) {
			...
			glGetActiveUniform(m_id, index, static_cast<GLsizei>(variable_name.size()), &name_length, &variable.size,
			                   &variable.type, &variable_name[0]);
	...
	{ // discover attributes
		...
		for (GLuint index = 0; index < static_cast<GLuint>(attribute_count); ++index) {
			...
			GLsizei name_length = 0;
			glGetActiveAttrib(m_id, index, static_cast<GLsizei>(variable_name.size()), &name_length, &variable.size,
			                  &variable.type, &variable_name[0]);
			assert(variable.type);
			assert(variable.size);
