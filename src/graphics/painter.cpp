...
std::shared_ptr<Texture2> Painter_Old::test_texture = {};

void Painter_Old::test()
{
...
void Painter_Old::drawSlider(const Aabrf& rect, float pos)
{
    ...
    m_cell.set_winding(Winding::HOLE);
    m_cell.set_fill_paint(bg);
    m_cell.fill(m_context);
...
void Painter_Old::drawButton(const Aabrf& rect)
{
...
void Painter_Old::drawCheckBox(const Aabrf& rect)
{
...
void Painter_Old::drawColorwheel(const Aabrf& rect, float t)
{
    ...
    for (int i = 0; i < 6; i++) {
        ...
        m_cell.begin_path();
        m_cell.arc(rect.x(), rect.y(), inner_radius, a0, a1, Winding::CLOCKWISE);
        m_cell.arc(rect.x(), rect.y(), outer_radius, a1, a0, Winding::COUNTERCLOCKWISE);
        m_cell.close_path();
        ...
        m_cell.set_fill_paint(Paint::create_linear_gradient(start_pos, end_pos, start_color, end_color));
        m_cell.fill(m_context);
    }
    ...
    m_cell.set_fill_paint(Paint::create_box_gradient(center, extend, 2, 4, Color(0, 0, 0, .5), Color(0, 0, 0, 0)));
    m_cell.fill(m_context);
    ...
    m_cell.set_winding(Winding::HOLE);
    m_cell.set_fill_paint(Paint::create_radial_gradient(Vector2f{ax, ay}, 7, 9, Color(0, 0, 0, 64), Color(0, 0, 0, 0)));
    m_cell.fill(m_context);
...
void Painter_Old::drawEyes(const Aabrf& rect, const Vector2f& target, float t)
{
...
void Painter_Old::drawGraph(const Aabrf& rect, float t)
{
    float samples[6];
    float sx[6], sy[6];
...
void Painter_Old::drawSpinner(const Vector2f& center, const float radius, float t)
{
    float a0 = 0.0f + t * 6;
    float a1 = PI + t * 6;
    float r0 = radius;
    ...
    m_cell.begin_path();
    m_cell.arc(center.x, center.y, r0, a0, a1, Winding::CW);
    m_cell.arc(center.x, center.y, r1, a1, a0, Winding::CCW);
    m_cell.close_path();
...
void Painter_Old::drawCaps(const Vector2f& pos, const float width)
{
    LineCap caps[3] = {LineCap::BUTT, LineCap::ROUND, LineCap::SQUARE};
...
void Painter_Old::drawJoins(const Aabrf& rect, const float time)
{
    LineJoin joins[3] = {LineJoin::MITER, LineJoin::ROUND, LineJoin::BEVEL};
    LineCap caps[3]   = {LineCap::BUTT, LineCap::ROUND, LineCap::SQUARE};
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            ...
            m_cell.set_line_cap(LineCap::BUTT);
            m_cell.set_line_join(LineJoin::BEVEL);
...
void Painter_Old::drawTexture(const Aabrf& rect)
{
    if (!test_texture) {
