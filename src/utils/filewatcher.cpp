...
public: // methods
    FileWatcher()
        : m_notifier(s_CLOSED), m_watcher(s_CLOSED) {}
    ...
    void start_watching(const std::string& directory, const Events events)
    {
        ...
        if (m_notifier == s_CLOSED) {
            switch (errno) {
            case EMFILE:
                throw std::runtime_error("The per-process limit on the number of open file descriptors has been reached.");
            case ENFILE:
        ...
        if (m_watcher == s_CLOSED) {
            ...
            case EINVAL:
                throw std::runtime_error("The given event mask contains no valid events, or `m_notifier` is not an inotify file descriptor");
            case ENAMETOOLONG:
                throw std::runtime_error("Pathname is too long");
            case ENOENT:
                throw std::runtime_error("A directory component in pathname does not exist or is a dangling symbolic link");
            case ENOMEM:
...
//    std::thread watcher(&FileWatcher::start_watching, &file_watcher, "/home/clemens/code/scratch/test/", FileWatcher::ALL);
//    std::cin.get(); // wait for an `enter`
