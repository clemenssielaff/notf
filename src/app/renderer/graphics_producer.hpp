...
///
/// We need to make sure that the GraphicsProducer is properly registered with its LayerManager. For that, we delegate
...
/// mutable LayerManagerPtr reference as the second:
///
/// ```
///     MyProducer(const Token& token, LayerManagerPtr& manager, bool myVeryOwnParameter, ...);
/// ```
///
/// The remaining parameters can be whatever.
///
/// The user creates instances via a factory method, that requires  the LayerManager as the first argument and
/// forwards all additional arguments to the subclass' constructor.
///
/// ```
///     static std::shared_ptr<MyProducer> create(LayerManagerPtr& manager, bool myVeryOwnParameter, ...)
///     {
...
protected:
    ...
    GraphicsProducer(const Token&) : m_id(_next_id()) {} // TODO: apply Token idiom to Item hierarchy
    ...
    /// This method will in turn register the new instance with the LayerManager.
    template<typename T, typename... Ts>
    static std::shared_ptr<T> _create(LayerManagerPtr& render_manager, Ts&&... args)
    {
...
#endif
        LayerManager::Access<GraphicsProducer>(*render_manager).register_new(result);
        return result;
    }
...
private:
    ...
    /// Is only callable from the LayerManager.
    void render() const;
