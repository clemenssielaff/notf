...
struct GlfwEventHandler {
    ...
    on_token_key(GLFWwindow* glfw_window, const int key, const int scancode, const int action, const int modifiers) {
        if (action == GLFW_PRESS) {
            schedule<KeyPressEvent>(glfw_window, to_key(key), scancode, modifiers);
        } else if (action == GLFW_RELEASE) {
            schedule<KeyReleaseEvent>(glfw_window, to_key(key), scancode, modifiers);
        } else {
            NOTF_ASSERT(action == GLFW_REPEAT);
            schedule<KeyRepeatEvent>(glfw_window, to_key(key), scancode, modifiers);
        }
    }
    ...
    static void on_shortcut(GLFWwindow* glfw_window, const uint codepoint, const int modifiers) {
        schedule<ShortcutEvent>(glfw_window, codepoint, modifiers);
    }
