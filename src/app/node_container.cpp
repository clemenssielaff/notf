...
{
    if (contains(node->get_name())) {
        return false;
    }
    m_names.emplace(std::make_pair(node->get_name(), node.raw()));
...
void NodeContainer::stack_before(const size_t index, valid_ptr<const Node*> sibling)
{
    ...
    if (sibling_it == m_order.end()) {
        NOTF_THROW(Scene::hierarchy_error,
                   ...
                   (*node_it)->get_name(), sibling->get_name());
    }
...
{
    auto name_it = m_names.find(node->get_name());
