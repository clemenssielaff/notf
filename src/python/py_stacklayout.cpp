...
void produce_stack_layout(pybind11::module& module, py::detail::generic_type ancestor)
{
    ...
    Py_StackLayout.def("get_content_alignment", &StackLayout::get_content_alignment, "Cross alignment the entire content if the Layout wraps.");
    Py_StackLayout.def("get_wrap", &StackLayout::get_wrap, "How (and if) overflowing lines are wrapped.");
    ...
    Py_StackLayout.def("get_cross_spacing", &StackLayout::get_cross_spacing, "Spacing between stacks of items if this Layout is wrapped.");
    ...
    Py_StackLayout.def("set_content_alignment", &StackLayout::set_content_alignment, "Defines the cross alignment the entire content if the Layout wraps.", py::arg("alignment"));
    Py_StackLayout.def("set_wrap", &StackLayout::set_wrap, "Defines how (and if) overflowing lines are wrapped.", py::arg("wrap"));
