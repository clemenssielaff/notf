...
from tests.test_classes import EmptyNode, AdderNode, StructuredBuffer
...
class TestNodeRegistry(unittest.TestCase):
    ...
    #
    #     node1.property("out").value = StructuredBuffer.create(2)
    #     self.assertEqual(output.value.read().as_number(), 2.0)
    #
    #     node2.property("out").value = StructuredBuffer.create(6)
