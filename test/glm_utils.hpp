...
inline void compare_mat4(const notf::Xform3f& my, const glm::mat4& their)
{
    for (size_t col = 0; col < 4; ++col) {
        for (size_t row = 0; row < 4; ++row) {
            REQUIRE(std::abs(my[col][row] - their[static_cast<int>(col)][static_cast<int>(row)]) < notf::precision_high<float>());
        }
    }
}

inline void compare_mat2(const notf::Xform2f& my, const glm::mat4& their)
{
    for (size_t col = 0; col < 2; ++col) {
        REQUIRE(std::abs(my[col][0] - their[static_cast<int>(col)][static_cast<int>(0)]) < notf::precision_high<float>());
        REQUIRE(std::abs(my[col][1] - their[static_cast<int>(col)][static_cast<int>(1)]) < notf::precision_high<float>());
