...
SCENARIO("signals are dynamic callbacks between functions and methods", "[common][signal]") {
	...
	WHEN("you connect a signal to a member function that is const") {
		...
		THEN("it woks as well (should, if compiles)") { REQUIRE(receiver.const_counter == 1); }
	}
	...
	WHEN("you connect a signal to a lambda function with a test function") {
		...
		sender.two_bool_signal.connect(
		    [&](bool a, bool b) {
			    if (a && b)
				    ++lambda_two_bool_counter;
		    },
		    [](bool a, bool b) { return a == b; });
	...
	WHEN("you connect a signal to a method without arguments") {
		...
		THEN("the slot is called as if the signals' signatures matched") { REQUIRE(receiver.void_counter == 4); }
	}
	...
	WHEN("you disable and enable specific connections from a signal") {
		...
		Connection a
		    = receiver.connect_signal(sender.int_signal, &Receiver::on_int_signal, [](int i) { return i == 3; });
		Connection b
		    = receiver.connect_signal(sender.int_signal, &Receiver::on_int_signal, [](int i) { return i == 5; });
