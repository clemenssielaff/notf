...
#define CATCH_INTERNAL_UNSUPPRESS_PARENTHESES_WARNINGS _Pragma("GCC diagnostic pop")
#endif
...
#if (defined _MSC_VER && _MSC_VER >= 1300) || (defined __GNUC__ && __GNUC__ >= 4 && __GNUC_MINOR__ >= 3) \
...
#define INTERNAL_CATCH_METHOD_AS_TEST_CASE(QualifiedMethod, ...)                                             \
    namespace {                                                                                              \
    Catch::AutoReg                                                                                           \
        INTERNAL_CATCH_UNIQUE_NAME(autoRegistrar)(&QualifiedMethod, "&" #QualifiedMethod,                    \
...
#define INTERNAL_CATCH_TEST_CASE_METHOD2(TestName, ClassName, ...)                                              \
    ...
    Catch::AutoReg                                                                                              \
        INTERNAL_CATCH_UNIQUE_NAME(autoRegistrar)(&TestName::test, #ClassName, Catch::NameAndDesc(__VA_ARGS__), \
...
#define INTERNAL_CATCH_TESTCASE2(TestName, Name, Desc)                                                                 \
    ...
    namespace {                                                                                                        \
    Catch::AutoReg                                                                                                     \
        INTERNAL_CATCH_UNIQUE_NAME(autoRegistrar)(&TestName, CATCH_INTERNAL_LINEINFO, Catch::NameAndDesc(Name, Desc)); \
    }                                                                                                                  \
    static void TestName()
...
#define INTERNAL_CATCH_TEST_CASE_METHOD2(TestCaseName, ClassName, TestName, Desc)                                      \
    ...
    Catch::AutoReg                                                                                                     \
        INTERNAL_CATCH_UNIQUE_NAME(autoRegistrar)(&TestCaseName::test, #ClassName, Catch::NameAndDesc(TestName, Desc), \
                                                  CATCH_INTERNAL_LINEINFO);                                            \
    }                                                                                                                  \
    void TestCaseName::test()
#define INTERNAL_CATCH_TEST_CASE_METHOD(ClassName, TestName, Desc)                                                  \
    INTERNAL_CATCH_TEST_CASE_METHOD2(INTERNAL_CATCH_UNIQUE_NAME(____C_A_T_C_H____T_E_S_T____), ClassName, TestName, \
                                     Desc)
...
struct MatchNotOf : MatcherBase<ArgT> {
    ...
    virtual bool match(ArgT const& arg) const CATCH_OVERRIDE { return !m_underlyingMatcher.match(arg); }

    virtual std::string describe() const CATCH_OVERRIDE { return "not " + m_underlyingMatcher.toString(); }
...
struct Evaluator<T1, T2, IsEqualTo> {
    static bool evaluate(T1 const& lhs, T2 const& rhs) { return bool(opCast(lhs) == opCast(rhs)); }
};
template<typename T1, typename T2>
...
public:
    ...
    virtual void reconstructExpression(std::string& dest) const CATCH_OVERRIDE { dest = Catch::toString(m_truthy); }
...
public:
    ...
    void endExpression() const { m_rb.setResultType(Internal::compare<Op>(m_lhs, m_rhs)).endExpression(*this); }
...
#define INTERNAL_CATCH_TEST(expr, resultDisposition, macroName)                                           \
    ...
    } while (Catch::isTrue(false && static_cast<bool>(!!(expr)))) // expr here is never evaluated at runtime but it
...
#define INTERNAL_CATCH_THROWS_AS(expr, exceptionType, resultDisposition, macroName)                       \
    do {                                                                                                  \
        Catch::ResultBuilder __catchResult(macroName, CATCH_INTERNAL_LINEINFO, #expr ", " #exceptionType, \
                                           resultDisposition);                                            \
        if (__catchResult.allowThrows())                                                                  \
            try {                                                                                         \
                static_cast<void>(expr);                                                                  \
...
struct ContainsElementMatcher : MatcherBase<std::vector<T>, T> {
    ...
    virtual std::string describe() const CATCH_OVERRIDE { return "Contains: " + Catch::toString(m_comparator); }
...
struct EqualsMatcher : MatcherBase<std::vector<T>, std::vector<T>> {
    ...
    virtual std::string describe() const CATCH_OVERRIDE { return "Equals: " + Catch::toString(m_comparator); }
    std::vector<T> const& m_comparator;
};
...
#endif
#if defined(CLARA_INTERNAL_CONFIG_CPP11_GENERATED_METHODS) && !defined(CLARA_CONFIG_CPP11_NO_GENERATED_METHODS) \
    && !defined(CLARA_CONFIG_CPP11_GENERATED_METHODS) && !defined(CLARA_CONFIG_NO_CPP11)
...
char toLowerCh(char c) { return static_cast<char>(::tolower(c)); }
...
inline void forceColour(ConfigData& config) { config.useColour = UseColour::Yes; }
...
struct TestRunStats {
    ...
    TestRunStats(TestRunStats const& _other) : runInfo(_other.runInfo), totals(_other.totals), aborting(_other.aborting)
    {
    }
#else
...
inline std::size_t listReporters(Config const& /*config*/)
{
    ...
    for (it = itBegin; it != itEnd; ++it) {
        ...
        Catch::cout()
            << "  " << it->first << ':' << std::string(maxNameLen - it->first.size() + 2, ' ') << wrapper << '\n';
    }
    Catch::cout() << std::endl;
...
public:
    TrackerBase(NameAndLocation const& nameAndLocation, TrackerContext& ctx, ITracker* parent)
        : m_nameAndLocation(nameAndLocation), m_ctx(ctx), m_parent(parent), m_runState(NotStarted)
    {
    }
    virtual ~TrackerBase();

    virtual NameAndLocation const& nameAndLocation() const CATCH_OVERRIDE { return m_nameAndLocation; }
    virtual bool isComplete() const CATCH_OVERRIDE
    {
        return m_runState == CompletedSuccessfully || m_runState == Failed;
    }
    virtual bool isSuccessfullyCompleted() const CATCH_OVERRIDE { return m_runState == CompletedSuccessfully; }
    virtual bool isOpen() const CATCH_OVERRIDE { return m_runState != NotStarted && !isComplete(); }
    virtual bool hasChildren() const CATCH_OVERRIDE { return !m_children.empty(); }

    virtual void addChild(Ptr<ITracker> const& child) CATCH_OVERRIDE { m_children.push_back(child); }
...
private:
    void runCurrentTest(std::string& redirectedCout, std::string& redirectedCerr)
    {
        ...
        try {
            m_lastAssertionInfo
                = AssertionInfo("TEST_CASE", testCaseInfo.lineInfo, std::string(), ResultDisposition::Normal);
...
public: // IRegistryHub
    RegistryHub() {}
    virtual IReporterRegistry const& getReporterRegistry() const CATCH_OVERRIDE { return m_reporterRegistry; }
    virtual ITestCaseRegistry const& getTestCaseRegistry() const CATCH_OVERRIDE { return m_testCaseRegistry; }
...
public: // IMutableRegistryHub
    ...
    virtual void registerTest(TestCase const& testInfo) CATCH_OVERRIDE { m_testCaseRegistry.registerTest(testInfo); }
...
std::string AssertionResult::getExpandedExpression() const { return m_resultData.reconstructExpression(); }
...
void AssertionResult::discardDecomposedExpression() const { m_resultData.decomposedExpression = CATCH_NULL; }

void AssertionResult::expandDecomposedExpression() const { m_resultData.reconstructExpression(); }
...
unsigned int Timer::getElapsedMicroseconds() const { return static_cast<unsigned int>(getCurrentTicks() - m_ticks); }
...
std::string toString(wchar_t* const value) { return Catch::toString(static_cast<const wchar_t*>(value)); }
...
std::string toString(unsigned int value) { return Catch::toString(static_cast<unsigned long>(value)); }
...
void ResultBuilder::reconstructExpression(std::string& dest) const { dest = m_assertionInfo.capturedExpression; }
...
public: // IStreamingReporter
    virtual ReporterPreferences getPreferences() const CATCH_OVERRIDE { return m_reporters[0]->getPreferences(); }
...
struct StreamingReporterBase : SharedImpl<IStreamingReporter> {
    ...
    virtual void testRunStarting(TestRunInfo const& _testRunInfo) CATCH_OVERRIDE { currentTestRunInfo = _testRunInfo; }
    virtual void testGroupStarting(GroupInfo const& _groupInfo) CATCH_OVERRIDE { currentGroupInfo = _groupInfo; }

    virtual void testCaseStarting(TestCaseInfo const& _testInfo) CATCH_OVERRIDE { currentTestCaseInfo = _testInfo; }
    virtual void sectionStarting(SectionInfo const& _sectionInfo) CATCH_OVERRIDE
    {
        m_sectionStack.push_back(_sectionInfo);
    }

    virtual void sectionEnded(SectionStats const& /* _sectionStats */) CATCH_OVERRIDE { m_sectionStack.pop_back(); }
...
public:
    ...
    public:
        ...
        ScopedElement(ScopedElement const& other) : m_writer(other.m_writer) { other.m_writer = CATCH_NULL; }
    ...
    XmlWriter() : m_tagIsOpen(false), m_needsNewline(false), m_os(Catch::cout()) { writeDeclaration(); }

    XmlWriter(std::ostream& os) : m_tagIsOpen(false), m_needsNewline(false), m_os(os) { writeDeclaration(); }
...
private:
    ...
    void writeDeclaration() { m_os << "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"; }
...
public:
    XmlReporter(ReporterConfig const& _config)
        : StreamingReporterBase(_config), m_xml(_config.stream()), m_sectionDepth(0)
    {
...
#define CATCH_REQUIRE_FALSE(expr)                                                                     \
    INTERNAL_CATCH_TEST(expr, Catch::ResultDisposition::Normal | Catch::ResultDisposition::FalseTest, \
                        "CATCH_REQUIRE_FALSE")

#define CATCH_REQUIRE_THROWS(expr) \
    INTERNAL_CATCH_THROWS(expr, Catch::ResultDisposition::Normal, "", "CATCH_REQUIRE_THROWS")
#define CATCH_REQUIRE_THROWS_AS(expr, exceptionType) \
    INTERNAL_CATCH_THROWS_AS(expr, exceptionType, Catch::ResultDisposition::Normal, "CATCH_REQUIRE_THROWS_AS")
#define CATCH_REQUIRE_THROWS_WITH(expr, matcher) \
    INTERNAL_CATCH_THROWS(expr, Catch::ResultDisposition::Normal, matcher, "CATCH_REQUIRE_THROWS_WITH")
...
#define CATCH_CHECK_FALSE(expr)                                                                                  \
    INTERNAL_CATCH_TEST(expr, Catch::ResultDisposition::ContinueOnFailure | Catch::ResultDisposition::FalseTest, \
                        "CATCH_CHECK_FALSE")
...
#define CATCH_CHECK_NOFAIL(expr)                                                                                    \
    INTERNAL_CATCH_TEST(expr, Catch::ResultDisposition::ContinueOnFailure | Catch::ResultDisposition::SuppressFail, \
                        "CATCH_CHECK_NOFAIL")
...
#define CATCH_CHECK_THROWS_AS(expr, exceptionType) \
    INTERNAL_CATCH_THROWS_AS(expr, exceptionType, Catch::ResultDisposition::ContinueOnFailure, "CATCH_CHECK_THROWS_AS")
#define CATCH_CHECK_THROWS_WITH(expr, matcher) \
    INTERNAL_CATCH_THROWS(expr, Catch::ResultDisposition::ContinueOnFailure, matcher, "CATCH_CHECK_THROWS_WITH")
#define CATCH_CHECK_NOTHROW(expr) \
    INTERNAL_CATCH_NO_THROW(expr, Catch::ResultDisposition::ContinueOnFailure, "CATCH_CHECK_NOTHROW")

#define CATCH_CHECK_THAT(arg, matcher) \
    INTERNAL_CHECK_THAT(arg, matcher, Catch::ResultDisposition::ContinueOnFailure, "CATCH_CHECK_THAT")
...
#define CATCH_WARN(msg) \
    INTERNAL_CATCH_MSG(Catch::ResultWas::Warning, Catch::ResultDisposition::ContinueOnFailure, "CATCH_WARN", msg)
...
#define CATCH_FAIL(...) \
    INTERNAL_CATCH_MSG(Catch::ResultWas::ExplicitFailure, Catch::ResultDisposition::Normal, "CATCH_FAIL", __VA_ARGS__)
#define CATCH_SUCCEED(...) \
    INTERNAL_CATCH_MSG(Catch::ResultWas::Ok, Catch::ResultDisposition::ContinueOnFailure, "CATCH_SUCCEED", __VA_ARGS__)
#else
...
#define CATCH_FAIL(msg) \
    INTERNAL_CATCH_MSG(Catch::ResultWas::ExplicitFailure, Catch::ResultDisposition::Normal, "CATCH_FAIL", msg)
#define CATCH_SUCCEED(msg) \
    INTERNAL_CATCH_MSG(Catch::ResultWas::Ok, Catch::ResultDisposition::ContinueOnFailure, "CATCH_SUCCEED", msg)
#endif
...
#define REQUIRE_FALSE(expr) \
    INTERNAL_CATCH_TEST(expr, Catch::ResultDisposition::Normal | Catch::ResultDisposition::FalseTest, "REQUIRE_FALSE")
...
#define WARN(msg) \
    INTERNAL_CATCH_MSG(Catch::ResultWas::Warning, Catch::ResultDisposition::ContinueOnFailure, "WARN", msg)
...
#define FAIL(...) \
    INTERNAL_CATCH_MSG(Catch::ResultWas::ExplicitFailure, Catch::ResultDisposition::Normal, "FAIL", __VA_ARGS__)
#define SUCCEED(...) \
    INTERNAL_CATCH_MSG(Catch::ResultWas::Ok, Catch::ResultDisposition::ContinueOnFailure, "SUCCEED", __VA_ARGS__)
#else
...
#define SUCCEED(msg) \
    INTERNAL_CATCH_MSG(Catch::ResultWas::Ok, Catch::ResultDisposition::ContinueOnFailure, "SUCCEED", msg)
#endif
