...
SCENARIO("A FlexLayout places its children without wrapping #1", "[dynamic][layout]")
{
    std::shared_ptr<FlexLayout> flexlayout = FlexLayout::create();
    flexlayout->set_claim(Claim::fixed(400, 400));

    WHEN("you add multiple widgets to a no-wrap overlayout with cross-alignment set to 'start'")
    {
