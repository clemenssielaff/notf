...
public:
    ...
    const Size2f& get_grant() const { return m_grant; } // TODO: is get_grant even used, now that we have the size?
    ...
    {
        return get_xform<space>().transform(Aabrf(m_size));
    }
...
private:
    ...
    void _get_window_transform(Matrix3f& result) const;
...
private:
    ...
    /// If a RenderLayer is explicitly set, this flag is set to true, so moving the ScreenItem to another parent will
    /// not change the RenderLayer.
...
template<>
const Matrix3f ScreenItem::get_xform<ScreenItem::Space::LOCAL>() const
{
    return Matrix3f::identity();
}

template<>
const Matrix3f ScreenItem::get_xform<ScreenItem::Space::OFFSET>() const
{
    return m_offset_transform;
}

template<>
const Matrix3f ScreenItem::get_xform<ScreenItem::Space::LAYOUT>() const
{
    return m_layout_transform;
}

template<>
const Matrix3f ScreenItem::get_xform<ScreenItem::Space::PARENT>() const
{
