...
public:
    ...
    void update(std::vector<index_t>&& data)
    {
        ...
        if (m_size <= m_buffer_size) {
            ...
            gl_check(glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, /*offset = */ 0, m_size * sizeof(index_t), &m_indices[0]));
        }
        else {
            // otherwise we have to do a full update
            gl_check(glBufferData(GL_ELEMENT_ARRAY_BUFFER, m_indices.size() * sizeof(index_t), &m_indices[0], m_args.usage));
        }
        // keep the buffer bound as it is stored in the VAO
