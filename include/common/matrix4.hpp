...
struct _Matrix4 : public detail::Arithmetic<_Matrix4<REAL, true>, _RealVector4<REAL>, 4> {
    ...
    _Matrix4(const element_t a, const element_t b, const element_t c, const element_t d,
             const element_t e, const element_t f, const element_t g, const element_t h,
             const element_t i, const element_t j, const element_t k, const element_t l,
             const element_t m, const element_t n, const element_t o, const element_t p)
        : super_t{component_t(a, b, c, d),
                  component_t(e, f, g, h),
                  component_t(i, j, k, l),
                  component_t(m, n, o, p)} {}
    ...
    static _Matrix4 translation(const _RealVector2<element_t>& t)
    {
        return _Matrix4::translation(t.x(), t.y());
    }

    /// @brief A 3d translation matrix.
    /// @param translation  3D translation vector.
    static _Matrix4 translation(const _RealVector3<element_t>& t)
    {
        return _Matrix4::translation(t.x(), t.y(), t.z());
    }
    ...
    static _Matrix4 scaling(const element_t s)
    {
        return _Matrix4(s, 0, 0, 0,
                        0, s, 0, 0,
                        0, 0, s, 0,
                        0, 0, 0, 1);
    }
    ...
    static _Matrix4 perspective(const element_t fov, const element_t aspect, element_t near, element_t far)
    {
        ...
        if (std::abs(aspect) <= precision_high<element_t>()
            || std::abs(far - near) <= precision_high<element_t>()) {
            return result;
        }
                                 ...
                                 const element_t top, element_t near, element_t far)
    {
        ...
        if (std::abs(width) <= precision_high<element_t>()
            || std::abs(height) <= precision_high<element_t>()
