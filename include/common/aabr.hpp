...
struct _Aabr {
     ...
     * @param height    Height of the Aabr.
     */
    _Aabr(const value_t width, const value_t height) : _min(0, 0), _max(width, height) {}

    /** Constructs an Aabr of the given size with the bottom-left corner at zero.
     * @param size  Size of the Aabr.
     */
    _Aabr(const _Size2<value_t>& size) : _min(0, 0), _max(size.width, size.height) {}
    ...
    _Aabr intersect(const _Aabr& other) const
    {
        ...
        return _Aabr(vector_t{_min.x() > other._min.x() ? _min.x() : other._min.x(),
