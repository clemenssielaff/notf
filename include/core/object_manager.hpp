...
public: // methods
    ...
    std::shared_ptr<Object> get_object(const Handle handle) const { return _get_abstract_object(handle); }
    ...
    {
        std::shared_ptr<Object> abstract_object = _get_abstract_object(handle);
        if (!abstract_object) {
            return {};
        }
        std::shared_ptr<T> cast_object = std::dynamic_pointer_cast<T>(abstract_object);
        if (!cast_object) {
            _wrong_type_warning(typeid(T).name(), handle);
        }
        return cast_object;
    }
...
private: // methods
    ...
    std::shared_ptr<Object> _get_abstract_object(const Handle handle) const;

    /// @brief Wrapper for logging.
    void _wrong_type_warning(const std::string& type_name, const Handle handle) const;
