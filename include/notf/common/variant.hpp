...
/// Template struct to test whether a given type is a variant or not.
template<class T>
...
constexpr std::enable_if_t<is_variant_v<Variant>, size_t> get_first_index() noexcept {
    NOTF_ASSERT(I != std::variant_size_v<Variant>);
    if constexpr (std::is_same_v<std::variant_alternative_t<I, Variant>, T>) {
        return I;
    } else {
        return get_first_index<T, Variant, I + 1>();
    }
}

/// Checks if a given type is part of the Variant.
template<class T, class Variant, size_t I = 0>
constexpr bool is_one_of_variant() noexcept {
...
template<class Variant>
NOTF_UNUSED constexpr bool has_variant_unique_types() noexcept {
