...
#include "notf/graphic/prefab.hpp"
#include "notf/graphic/prefab_group.hpp"
...
public:
    ...
    struct Box : public Definition {
        V3d center = V3d::zero();
        V3d up_axis = V3d::y_axis();
        V3d orient_axis = V3d::x_axis();
...
protected:
    ...
    static void _convert(const V2d& in, V2f& out);
    static void _convert(const V2d& in, V2h& out);
    static void _convert(const V3d& in, V3f& out);
    static void _convert(const V3d& in, V3h& out);
    static void _convert(const V3d& in, V4f& out);
    static void _convert(const V3d& in, V4h& out);

    /// Error overloads for _convert to give helpful error messages.
    template<typename UNSUPPORTED_TYPE>
    static void _convert(const V3d&, UNSUPPORTED_TYPE&) {
        static_assert(always_false_v<UNSUPPORTED_TYPE>, "No conversion defined from V3d to UNSUPPORTED_TYPE");
    }
    template<typename UNSUPPORTED_TYPE>
    static void _convert(const V2d&, UNSUPPORTED_TYPE&) {
...
public:
    ...
    std::shared_ptr<PrefabType<InstanceData>> produce(std::string name) {
        ...
        for (const std::unique_ptr<Definition>& def : m_definitions) {
            switch (def->kind()) {
            case Kind::BOX: _ingest_product(_produce(*static_cast<Box*>(def.get()))); break;
            case Kind::SPHERE: _ingest_product(_produce(*static_cast<Sphere*>(def.get()))); break;
            }
        }
