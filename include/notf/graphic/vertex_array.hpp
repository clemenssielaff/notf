...
struct is_attribute_trait<
    T, std::void_t<
           ...
           typename T::kind,
           std::enable_if_t<is_one_of_v<typename T::kind, AttributeKind::Position, AttributeKind::Normal,
                                        AttributeKind::Color, AttributeKind::TexCoord, AttributeKind::Other>>>>
    : std::true_type {};
...
public:
    ...
    /// Destructor.
    virtual ~VertexArrayType() { NOTF_CHECK_GL(glDeleteBuffers(1, &m_vbo_id)); }
...
public:
    ...
    void init() {
        { // make sure there is a bound VAO
            ...
            if (!current_vao) { NOTF_THROW(OpenGLError, "Cannot initialize a VertexArray without a bound VAO"); }
        }

        if (m_vbo_id) { return _update(); }

        NOTF_CHECK_GL(glGenBuffers(1, &m_vbo_id));
        if (!m_vbo_id) { NOTF_THROW(OpenGLError, "Failed to allocate VertexArray"); }
