...
public:
    ...
    /// Default constructor.
    PrefabGroup()
        : m_vertex_array(std::make_unique<vertex_array_t>()), m_index_array(std::make_unique<index_array_t>()) {
    ...
    void init() {
        if (m_vao_id) { NOTF_THROW(runtime_error, "Cannot re-initialize a previously initialized PrefabGroup."); }

        NOTF_CHECK_GL(glGenVertexArrays(1, &m_vao_id));
        if (!m_vao_id) { NOTF_THROW(runtime_error, "Failed to allocate the PrefabLibary VAO"); }
    ...
    std::shared_ptr<PrefabType<InstanceData>> prefab_type(const std::string& name) const {
        const auto it = std::find_if(m_prefab_types.begin(), m_prefab_types.end(),
                                     [&](const auto& prefab_type) -> bool { return prefab_type->name() == name; });
        if (it != m_prefab_types.end()) { return *it; }
